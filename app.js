...
import { getMainMenu, yesNoKeyboard } from './keywords'
import { getMyTasks, addTask, deleteTask } from './db.js'

const app = express()
const bot = new Telegraf('5640366062:AAHPjgd_nxh6aRkymwbhnLE5-JYZaaBahiU')

bot.use(session())

bot.start(ctx => {
    ctx.replyWithHTML(
        'Здорово это бот для задач по програмированию'+
        'Чтобы быстро получить задачу, просто напишите какой ЯП вы используете и отправьте боту',
        getMainMenu())
})
